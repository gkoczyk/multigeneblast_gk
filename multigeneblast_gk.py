...
## some quick adjustments from Grzegorz M. Koczyk (2020):
## - distance b/w genes up to 1000kb
## - do not ignore coverage parameter for alignments longer than 40
## - 
...
def blastparse(blasttext, minseqcoverage, minpercidentity, seqlengths, seqdict, dbname, dbtype):
  ...
  for i in blastlines:
    ...
    # GK: changed to query end - query start + 1...  was float(tabs[3]) - alignment length raw incl. gaps
    alignmentlength = max(float(tabs[7]), float(tabs[6]))- min(float(tabs[6]), float(tabs[7])) + 1.
    evalue = str(tabs[10])
    blastscore = int(tabs[11].split(".")[0])
    if seqlengths.has_key(query):
      # Ditto for alilen
      perc_coverage = (alignmentlength / seqlengths[query]) * 100
    else:
      perc_coverage = 0
      print seqlengths
      print "Error: no sequence length found for", query
      sys.exit()
    if perc_ident > minpercidentity and (perc_coverage > minseqcoverage and alignmentlength > 40): #or alignmentlength > 40):
      blastlines2.append(i)
  blastlines = blastlines2
  ...
  for i in blastlines:
    ...
    evalue = str(tabs[10])
    blastscore = int(tabs[11].split(".")[0])
    # GK: likewise corrected for alilen in relation to query (won't go over 100.)
    if seqlengths.has_key(query):
      perc_coverage = (alignmentlength / seqlengths[query]) * 100
    else:
      seqlength = len(seqdict[query.split("|")[4]])
      perc_coverage = (alignmentlength / seqlength) * 100
...
-singles <y/n>        : allow single genes to generate cluster matches (default: n)
"""
    ...
    for i in identifiers:
        if i == "-help" or i == "--help" or i == "-h":
          ...
          if i == "-from":
              ...
              if value.isdigit() and int(value) in range(1,1001):
                  opts.distancekb = int(value) * 1000
              else:
                  print "Error: please select a number between 1-1000."
                  invalidoptions(i)
              ...
              if value == "y" or value == "n":
                  opts.singles = True if value=="y" else False
              else:
                  invalidoptions(i)
          else:
              invalidoptions(i)
...
def load_dbproteins_info(querylist, blastdict, dbname, allow_singles=True):
  ...
  for j in allhitprots:
    ...
    # GK: fix to allow similarities based on solely a single gene (before it only kicked in if at least 2 hits)
    if 1 or allow_singles:
...
def load_databases(querylist, blastdict, processnr, dbname, dbtype, allow_singles=True):
  #Load GenBank positional info into memory
  log("Loading GenBank positional info into memory...")
  if dbtype == "prot":
    allgenomes, nucdict, proteininfo = load_dbproteins_info(querylist, blastdict, dbname, allow_singles=allow_singles)
...
def sort_genes_per_nucleotide(querylist, blastdict, nucdict, allow_singles=True):
  ...
  for j in nucdict.keys():
    nucsource = nucdict[j]
    # GK: fix to allow a single gene to generate a cluster
    if allow_singles:
      multiplelist.append(nucsource)
      if j not in multiplehitlist:
        multiplehitlist.append(j)
      if nucsource not in sourcedict:
        sourcedict[nucsource] = []
...
def find_genomic_loci(blastdict, nucdict, proteininfo, allowedgenedistance, querylist, nucdescriptions, dnaseqlength, allow_singles=True):
  ...
  sourcedict, multiplehitlist, universalquerydict = sort_genes_per_nucleotide(querylist, blastdict, nucdict,allow_singles=allow_singles)
...
def main():
  ...
  nucdescriptions, nucdict, proteininfo = load_databases(querylist, blastdict, opts.nrcpus, opts.db, opts.dbtype, allow_singles=opts.singles)
  print "Step 6/11: Time since start: " + str((time.time() - starttime))
  #Step 7: Locate Blast hits in genomes
  blastdict, geneposdict, hitclusters, clusters, multiplehitlist = find_genomic_loci(blastdict, nucdict, proteininfo, opts.distancekb, querylist, nucdescriptions, dnaseqlength, allow_singles=opts.singles)
